...
#ifndef MOTHERBOARD                            // chọn mạch chính
 ...
 */
#define SERIAL_PORT 0 // xác định cổng UART chính mà bo mạch chủ sử dụng làm giao tiếp serial chính 
 ...
 */
#define BAUDRATE 115200            // chon baudrate
// #define POWER_SUPPLY HIGH
 ...
 */
#define X_DRIVER_TYPE TB6600               // chon loai driver
#define Y_DRIVER_TYPE TB6600               // chon loai driver
#define Z_DRIVER_TYPE TB6600               // chon loai driver
...
// :[0, 1, 2, 3, 4, 5, 6, 7, 8]
#define EXTRUDERS 1             // so luong extruder ( dau phun )

// Generally expected filament diameter (1.75, 2.85, 3.0, ...). Used for Volumetric, Filament Width Sensor, etc.
#define DEFAULT_NOMINAL_FILAMENT_DIA 1.75   // duong kinh soi nhua in 
...
//#define MAGNETIC_PARKING_EXTRUDER
  ...
  #define PARKING_EXTRUDER_GRAB_DISTANCE 1            // (mm) Distance to move beyond the parking point to grab the extruder
    ...
    #define MPE_TRAVEL_DISTANCE   10      // (mm)
Last distance point
 ...
 */
#define TEMP_SENSOR_0 1    // cai dat cảm biến đầu phun, bàn nhiệt.
...
// Enable pullup for all endstops to prevent a floating state // KÍCH HOẠT PULLUP CHO TẤT CẢ CÁC ĐIỂM CUỐI ĐỂ NGĂN TRẠNG THÁI NỔI
...
// kích hoạt tính năng này nếu các chân đều có khả năng ngắt 
 ...
 * :[2,3,4,5,6,7]
 */
//Debounce và độ nhạy của công tắc.
#define ENDSTOP_NOISE_THRESHOLD 2 // Đặt ngưỡng chống nhiễu( tính bằng milisecond)
    ...
    */
   // thiết lâp cấu hình các trục ( số bước động cơ cần thiết để di chuyển 1mm)
#define DEFAULT_AXIS_STEPS_PER_UNIT {100, 100, 200, 93} // X Y Z E: là số bước/mm của trục đùn (extrude), có thể không cần dùng trong ứng dụng này
                                                         // SỐ BƯỚC MÀ ĐỘNG CƠ DI CHUYỂN TRÊN 1MM TRÊN X,Y,Z.
                                                         // Step per mm =(Số bước của động cơ trong 1 vòng quay* Mức phân chia bước)/(Số răng * Khoảng cách giữa 2 răng )
   /**
...
#define EDITABLE_STEPS_PER_UNIT
    ...
    *                                      X, Y, Z [, I [, J [, K...]]], E0 [, E1[, E2...]]
    */
   // TỐC ĐỘ VÀ GIA TỐC :CẤU HÌNH TỐC ĐỘ VÀ GIA TỐC TỐI ĐA CHO MỖI TRỤC ĐỂ ĐẢM BẢO ROBOT HOẠT ĐỘNG MƯỢT MÀ
...
#define MAX_ACCEL_EDIT_VALUES {6000, 6000, 200, 20000} // ...or, set your own edit limits
    ...
    */
#define DEFAULT_ACCELERATION 500          // Gia tốc
...
#define Z_CLEARANCE_DEPLOY_PROBE   10 // (mm) Z Clearance for Deploy/Stow
   // Đây là khoảng cách tối thiểu mà trục Z cần di chuyển lên trên khi triển khai hoặc thu lại cảm biến Z-probe. 
   //Trong trường hợp này, khi cảm biến được triển khai hoặc thu lại, trục Z sẽ di chuyển lên ít nhất 10mm so với điểm hiện tại
#define Z_CLEARANCE_BETWEEN_PROBES 5 // (mm) Z Clearance between probe points
   // Khoảng cách này xác định khoảng cách tối thiểu mà trục Z sẽ di chuyển lên giữa các điểm dò khi cảm biến thực hiện nhiều lần đo (multiple probes).
   // Khi thực hiện phép dò, trục Z sẽ di chuyển lên ít nhất 5mm giữa các lần dò điểm.
#define Z_CLEARANCE_MULTI_PROBE 5 // (mm) Z Clearance between multiple probes
   // Tương tự như trên, nhưng có thể được sử dụng khi bạn thực hiện nhiều lần dò ở các vị trí khác nhau trong quá trình hiệu chuẩn.
   // Khoảng cách này đảm bảo rằng trục Z không chạm vào vật thể trong quá trình di chuyển giữa các lần dò
#define Z_PROBE_ERROR_TOLERANCE 3 // (mm) Tolerance for early trigger (<= -probe.offset.z + ZPET)
   // Đây là độ lệch cho phép cho việc kích hoạt sớm của cảm biến Z-probe. Nếu cảm biến được kích hoạt quá sớm (khi trục Z chưa đạt đến vị trí cần thiết), Marlin sẽ cho phép sai số lên đến 3mm trước khi báo lỗi. 
   //Điều này giúp tránh việc đọc sai khi trục Z di chuyển quá nhanh
   //  #define Z_AFTER_PROBING           5 // (mm) Z position after probing is done // nó sẽ xác định vị trí Z mà trục Z sẽ di chuyển đến sau khi hoàn thành việc dò.
   //  Trong trường hợp này, Z sẽ di chuyển lên 5mm sau khi kết thúc quá trình dò.
...
#define DISABLE_OTHER_EXTRUDERS // Keep only the active extruder enabled
   ...
   // #define INVERT_X_DIR false          // CAI DAT CHIEU XOAY (HOME DIR)
   // #define INVERT_Y_DIR false          // giữ nguyên chiều quay
...
#define INVERT_E7_DIR false
   ...
   // #define Z_AFTER_HOMING         10   // (mm) // xét sau khi về home thì lùi lại 1 khoảng
   ...
   // KÍCH THƯỚC BÀN IN TRÊN TRỤC X.
   // KHOẢNG CÁCH TỐI ĐA TRƯỚC KHI CHẠM CÔNG TẮC
#define X_BED_SIZE 200
#define Y_BED_SIZE 200

   // Gioi hạn vùng làm việc.
#define X_MIN_POS 0
#define Y_MIN_POS 0
#define Z_MIN_POS 0
...
#define Z_MAX_POS 100
   ...
   // sử dụng giới hạn phần mềm thay vì kiểm tra trạng thái của công tắc hành trình
   ...
   // Điểm cuối phần mềm tối đa hạn chế chuyển động trong giới hạn tọa độ tối đa
...
#endif
   // TỐC ĐỘ VÀ GIA TỐC KHI HOMEING
   // Homing speeds (linear=mm/min, rotational=°/min)
#define HOMING_FEEDRATE_MM_M {(50 * 60), (50 * 60), (4 * 60)}
   // tốc độ di chuyển trục X,Y KHI GỌI LỆNH G28
...
// (Tùy chọn) Cấu hình home: Nếu bạn có cảm biến home để đưa robot về vị trí xuất phát, bạn cần cấu hình nó.
// CÔNG TẮC HÀNH TRÌNH THƯỜNG SỬ DỤNG TÍN HIỆU LOGIC NO(normally open) hoặc NC(Normally closed).Nếu Tín hiệu ngược với thực tế, sử dụng đoạn mã :
...
// #define X_MAX_ENDSTOP
// #define ENDSTOP_CONFIG(pin, inverting, hit_state) \
//   {                                               \
//     /* Logic để cấu hình pin */             \
//   }
// #define X_MAX_PIN 6
// #define Y_MAX_PIN 7
// #define Z_MAX_PIN 8
// #define LOW 0
#define X_MIN_ENDSTOP_HIT_STATE LOW // Trạng thái khi công tắc X min được kích hoạt
#define Y_MIN_ENDSTOP_HIT_STATE LOW // Trạng thái khi công tắc Y min được kích hoạt
#define Z_MIN_ENDSTOP_HIT_STATE LOW // Trạng thái khi công tắc Z min được kích hoạt
#define X_MAX_ENDSTOP_HIT_STATE LOW // Trạng thái khi công tắc X max được kích hoạt
#define Y_MAX_ENDSTOP_HIT_STATE LOW // Trạng thái khi công tắc Y max được kích hoạt
#define Z_MAX_ENDSTOP_HIT_STATE LOW // Trạng thái khi công tắc Z max được kích hoạt
//                                     // hoặc HIGH tuỳ vào trạng thái bạn cần đảo ngược

// // ENDSTOP_CONFIG(X_MAX_PIN, X_MAX_ENDSTOP_INVERTING, X_MAX_ENDSTOP_HIT_STATE) // Config cho X_MAX
// // ENDSTOP_CONFIG(Y_MAX_PIN, Y_MAX_ENDSTOP_INVERTING, Y_MAX_ENDSTOP_HIT_STATE) // Config cho Y_MAX
// // ENDSTOP_CONFIG(Z_MAX_PIN, Z_MAX_ENDSTOP_INVERTING, Z_MAX_ENDSTOP_HIT_STATE) //

// #define X_MIN_ENDSTOP_INVERTING false // Không đảo tín hiệu công tắc X min
// #define Y_MIN_ENDSTOP_INVERTING false // Không đảo tín hiệu công tắc Y min
// #define Z_MIN_ENDSTOP_INVERTING false // Không đảo tín hiệu công tắc Z min
// #define X_MAX_ENDSTOP_INVERTING false // Không đảo tín hiệu công tắc X max
// #define Y_MAX_ENDSTOP_INVERTING false // Không đảo tín hiệu công tắc Y max
// #define Z_MAX_ENDSTOP_INVERTING false // Không đảo tín hiệu công tắc Z max

// ENDSTOP_CONFIG(X_MAX_PIN, X_MAX_ENDSTOP_INVERTING, X_MAX_ENDSTOP_HIT_STATE) // Config cho X_MAX
// ENDSTOP_CONFIG(Y_MAX_PIN, Y_MAX_ENDSTOP_INVERTING, Y_MAX_ENDSTOP_HIT_STATE) // Config cho Y_MAX
// ENDSTOP_CONFIG(Z_MAX_PIN, Z_MAX_ENDSTOP_INVERTING, Z_MAX_ENDSTOP_HIT_STATE) //
// //  đảm bảo chiều quay của động cơ bước trên các trục.
#define INVERT_X_DIR true   // Giu nguyên chiều quay 
#define INVERT_Y_DIR true  // giữ nguyên chiều quay 
#define INVERT_Z_DIR true

// CẤU HÌNH HƯỚNG HOMEMING ( về điểm min của trục X,Y,Z)
#define X_HOME_DIR -1
#define Y_HOME_DIR -1
#define Z_HOME_DIR 1
//
// #define Z_OFFSET -100  // Đặt Z-offset là -100 để Z-max hiển thị là 0

// HẠN CHẾ DI CHUYỂN NGOÀI VÙNG CÔNG TẮC
